...
float r[] = {0.1,0.4,0.0,0.9,0.2,0.5,0.0,0.6,0.4,0.0};
float g[] = {0.2,0.0,0.3,0.5,0.2,0.0,0.3,0.4,0.1,0.2};
float b[] = {0.4,0.5,0.0,0.6,0.5,0.0,0.1,0.2,0.5,0.1};
int TIME=0;
bool START = false;
...
static void display(void)
{
    ...
    if(START){
        ...
        drawStrokeText("UP:W,DOWN:S,LEFT:A,RIGHT:D,ROUND:R,TOPVIEW:T,MAINMENU:M",-9,0.9,-0);
        ...
        drawStrokeText("SRINIVAS INSTITUTE OF TECHNOLOGY",-3,-0,-0);
        drawStrokeText("DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING",-4,-1,-0);
        drawStrokeText("COMPUTER GRAPHICS AND VISULIZATION",-3,-2,-0);
        drawStrokeText("A mini project on 3D PLANE GAME",-3,-3,-0);
        drawStrokeText("By",-0,-4,-0);
        drawStrokeText("RUKHIYATHUL HARSI",-3,-5,-0);
         drawStrokeText("4SN18CS075",-3,-6,-0);
        drawStrokeText("YASHASWINI R",1,-5,-0);
        drawStrokeText("4SN18CS106",1,-6,-0);
        drawStrokeText("Press G to Start",4,-7,-0);
       // drawStrokeText("Press X to Exit",4,-8,-0);
        drawStrokeText("PROJECT GUIDED BY",-7,-7,-0);
        drawStrokeText("Mr.Sudhakara.B",-7,-8,-0);
